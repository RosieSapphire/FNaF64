...
void office_update(const int button_state, struct update_params uparms)
{
        ...
        if (!camera_is_visible) {
                if (settings_flags & SET_SMOOTH_TURN_BIT) {
                        const int stick_clamped =
                                icutoff(uparms.sticks.stick_x, 10);
                        const float turn_amount =
                                stick_clamped * OFFICE_TURN_SPEED;
                        game_office_turn_lerp -= turn_amount * uparms.dt;
                        game_office_turn_lerp = CLAMP(game_office_turn_lerp,
                                                      OFFICE_TURN_MIN, 0);

                        /* FIXME: Use a fucking epsilon. */
                        if (fabsf(game_office_turn_lerp - game_office_turn) >=
                            0.001f) {
                                game_office_turn = lerpf(game_office_turn,
                                                         game_office_turn_lerp,
                                                         uparms.dt * 16);
                        }
                        ...
                        game_office_turn = CLAMP(game_office_turn,
                                                 OFFICE_TURN_MIN, 0);
                }
        }

        if (button_state & (GAME_DOOR_BTN_LEFT_LIGHT |
        ...
        game_office_flicker_rand_timer =
                wrapf(game_office_flicker_rand_timer + uparms.dt * 60,
                      1.0 / 60, &do_rand);
        if (do_rand)
