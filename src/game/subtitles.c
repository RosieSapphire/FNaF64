...
#define SUBS_FONT_ID      2
#define SUBS_NIGHT_CNT    5

static const int sub_night_line_nums[SUBS_NIGHT_CNT] = {
        63, 25, 19, 20, 13
};

static float sub_night_line_times[SUBS_NIGHT_CNT][SUBS_MAX_LINE_CNT] = {
        { // night 1
                0, // nothing
                3.4f, // ringing
                18.9f, // hello
...
static const char *sub_night_line_strs[SUBS_NIGHT_CNT][SUBS_MAX_LINE_CNT] = {
        { // night 1
                "",
                "*phone ringing*",
                "Hello? Uh, hello?",
...
void subtitles_load(void)
{
        ...
        rdpq_text_register_font(SUBS_FONT_ID, sub_font);
}
...
void subtitles_draw(const float timer, const int night, const float offset)
{
        ...
        for (i = 0; i < sub_night_line_nums[night - 1]; ++i) {
                float true_timer, line_cur_time;

                true_timer = timer + game_pause_timer_accum;
                line_cur_time = sub_night_line_times[night - 1][i] + offset;

                if (true_timer >= line_cur_time)
                        line_cur_index = i;
        }
        ...
        rdpq_sync_tile();
        rspq_wait();
        rdpq_text_printf(&params, SUBS_FONT_ID, 0, 48, "%s",
                         sub_night_line_strs[night - 1][line_cur_index]);
}

void subtitles_unload(void)
{
        rdpq_text_unregister_font(SUBS_FONT_ID);
