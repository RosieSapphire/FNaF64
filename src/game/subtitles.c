...
static const char *night_dialogue[5][MAX_LINES] = {
	...
	{ // night 5
	  "", "*phone ringing*", "*static*", "*demonic muttering*",
	  "*deep scream*", "*demonic muttering*", "*deep scream*",
	  "*demonic muttering*", "*deep scream*", "*demonic muttering*",
	  "*high-pitch screaming*", "*phone hangs up*", "" },
};

void subtitles_load(void)
{
	sub_font = rdpq_font_load("rom:/custom/debug_font.font64");
	rdpq_font_style(sub_font, 0,
			&(rdpq_fontstyle_t){
