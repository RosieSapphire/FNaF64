...
#define WHICH_NIGHT_TIMER_FADEOUT ( 70.f / TICKRATE)
#define WHICH_NIGHT_TIMER_END     (130.f / TICKRATE)
...
static void which_night_load(void)
{
        ...
        if (night_cur < 7) {
                const int *ais_night_cur = which_night_ai_lut[night_cur - 1];

                bonnie_ai_level = ais_night_cur[0];
                chica_ai_level  = ais_night_cur[1];
                foxy_ai_level   = ais_night_cur[2];
                freddy_ai_level = ais_night_cur[3];
        }
}
